...
def predict(dataset_name, input_path, output_path, max_image_size=640):
    if exists(output_path + ".npz") and exists(output_path + ".png"):
        return
    ...
    if image.shape[0] > max_image_size or image.shape[1] > max_image_size:
        scale = max_image_size / float(max(image.shape[0], image.shape[1]))
        image = cv2.resize(image, (int(scale * image.shape[1]),
    ...
    np.savez_compressed(output_path + ".npz", prob=prob)
    cv2.imwrite(output_path + ".png", color_image)
...
def main():
    ...
    for input_path in input_paths:
        output_path = "{}_{}".format(join(args.output_path,
                                          basename(splitext(input_path)[0])),
                                     args.dataset)
        print("Processing", input_path, output_path)
        predict(args.dataset, input_path, output_path)
